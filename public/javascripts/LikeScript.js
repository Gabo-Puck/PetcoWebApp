...
fetch(`/petco/publicacion/likes/${idPublicacion}/${idSession}/0`)
    .then((res) => res.json())
    .then((res) => {
        if (res.length >0 ) {
...
function heart() {
    //alert(idSession);
    if (ban == true) { //Cuando se indica que me gusta
        tolike.style.display = 'none';
        liked.style.display = 'block';
        ban = false;
        //Se realiza la query (Se crea el registro del like)
        fetch(`/petco/publicacion/likes/${idPublicacion}/${idSession}/1`)
            .then((res) => res.json())
            .then((res) => {
                console.log(res);
        ...
        fetch(`/petco/publicacion/likes/${idPublicacion}/${idSession}/2`)
            .then((res) => res.json())
            .then((res) => {
                console.log(res);
