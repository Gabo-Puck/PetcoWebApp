...
form.addEventListener("submit", (e) => {
  e.preventDefault();
  if (input.value) {
    //Se ejecuta lo siguiente al haber introducido el texto
...
socket.on("comentario", ({ msg, nombre, imagen, fecha, idcomentario }) => {
  ...
  document
    ...
    .insertBefore(tarjeta2, list.children[0]); //Se inserta el comentario dentro

  tarjeta2.querySelector("form").addEventListener("submit", (e) => {
    e.preventDefault();
    if (tarjeta2.querySelector("input").value) {
      //Se ejecuta lo siguiente al haber introducido el texto
      //Se envian los datos a la funcion
      var today = new Date();
      var date =
...
for (let i = 0; i < ncomentarios; i++) {
  //Se obtienen los formularios
  GetCommentsForm[i] = document.querySelector(
    "#comentarioForm-" + comentariosJS[i].ID
  );
  GetCommentsInput[i] = document.querySelector(
    "#inputC-" + comentariosJS[i].ID
  );
}
...
for (let i = 0; i < ncomentarios; i++) {
  ...
  GetCommentsForm[i].addEventListener("submit", (e) => {
    ...
    if (GetCommentsInput[i].value) {
      ...
      socket.emit("respuesta", {
        msg: GetCommentsInput[i].value,
        idP: idPublicacion,
        idcomentario: comentariosJS[i].ID,
        nombre: nombreuser,
        imagen: imagenperfil,
        fecha: dateTime,
        idDueno: idUsuarioPublicacion,
