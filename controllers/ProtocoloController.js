...
function createPromisePatch(paso) {
  ...
  return new Promise((resolve, reject) => {
    resolve(Paso.query().findById(paso.ID).patch(paso));
  });
}
...
function patchIntoBD(paso, req) {
  var promisePatch;
  return new Promise((resolve, reject) => {
    resolve(Paso.query().findById(paso.ID));
  })
    .then((PasoFound) => {
      return new Promise((resolve, reject) => {
        if (PasoFound != null) {
          // promisePatch = createPromisePatch(paso);
...
function createPromiseInsert(paso) {
  return new Promise((resolve, reject) => {
    resolve(Paso.query().insert(paso));
  });
}
...
function getPromisesUpsert(req, res, idProtocolo) {
  var PromisesUpsert = [];
  req.body["Pasos"].forEach((paso) => {
    paso["ID_Protocolo"] = idProtocolo;
    console.log(paso);
    if (paso.ID) {
      //promise patch
      PromisesUpsert.push(patchIntoBD(paso, req));
    } else {
      //promise insert
...
exports.ProtocoloEditarPost = [
  ...
  (req, res) => {
    Protocolo.query()
      ...
      .then((ProtocoloFetched) =>
        getPromisesUpsert(req, res, ProtocoloFetched.ID)
      )
      .then((promisesUpsert) => Promise.all(promisesUpsert))
      .then(() => uploadFiles(res))
      .then(() => deleteFiles(req))
